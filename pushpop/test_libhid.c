...
int main(int argc, char *argv[])
{
  ...
  unsigned short vendor_id  = 0xfeed;
  unsigned short product_id = 0xbeef;
  char *vendor, *product;
...
#define PATHLEN  2
   int PATH_IN[PATHLEN] = { 0x00010002, 0x00090001 };
#define SEND_PACKET_LEN 3
   // fill an example packet:
   char PACKET[SEND_PACKET_LEN] = { 0x0, 0x1, 0x2 };

   int i;
   for(i=0;i<3;i++)
   {
       ret = hid_get_input_report(hid, PATH_IN, PATHLEN, PACKET, SEND_PACKET_LEN);
       if (ret != HID_RET_SUCCESS) {
           fprintf(stderr, "hid_get_input_report failed with return code %d\n", ret);
       }

       printf("\n  *** %d ***  ",i);
       printf("PATH_IN = %X _ %X\n",PATH_IN[0],PATH_IN[1]);
       printf("PACKET[] = %X _ %X _ %X\n",PACKET[0],PACKET[1],PACKET[2],PACKET[3]);
       puts("  ********  \n");
       usleep(400);
   }
   PATH_IN[0] = 0x21090002;
   PATH_IN[1] = 0x00000300;

   char PACKET2[1] = { 0b00111000 };

   ret = hid_set_output_report(hid, PATH_IN, PATHLEN, PACKET2, 1);
   if (ret != HID_RET_SUCCESS) {
       fprintf(stderr, "hid_set_output_report failed with return code %d\n", ret);
   }
